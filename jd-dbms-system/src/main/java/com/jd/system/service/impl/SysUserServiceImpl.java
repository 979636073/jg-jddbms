...
public class SysUserServiceImpl implements ISysUserService {
    ...
    @Override
    public SysUser selectUserByUserAndPhone(String userName, String phone) {
        SysUser sysUser = userMapper.selectUserByUserAndPhone(userName, phone);
        setRoles(sysUser);
        return sysUser;
    }

    private void setRoles(SysUser sysUser) {
        if (sysUser == null) {
            return;
        }
        Long[] roleIds = null;
        if (CollUtil.isNotEmpty(sysUser.getRoles())) {
            roleIds = new Long[sysUser.getRoles().size()];
            for (int i = 0; i < sysUser.getRoles().size(); i++) {
                roleIds[i] = sysUser.getRoles().get(i).getRoleId();
            }
        }
