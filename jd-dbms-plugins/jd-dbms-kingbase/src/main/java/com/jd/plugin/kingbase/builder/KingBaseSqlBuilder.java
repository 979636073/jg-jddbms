package com.jd.plugin.kingbase.builder;

import com.jd.plugin.kingbase.type.KingBaseColumnTypeEnum;
import com.jd.plugin.kingbase.type.KingBaseIndexTypeEnum;
...
public class KingBaseSqlBuilder extends DefaultSqlBuilder {
    ...
    public String buildCreateTableSql(Table table) {
        ...
        List<TableColumn> tableColumnList = table.getColumnList().stream().filter(v -> StringUtils.isNotBlank(v.getComment())).collect(Collectors.toList());
        ...
        List<TableIndex> indexList = table.getIndexList().stream().filter(v -> StringUtils.isNotBlank(v.getComment())).collect(Collectors.toList());
    ...
    public String buildModifyTaleSql(Table oldTable, Table newTable) {
        ...
        newTable.setColumnList(newTable.getColumnList().stream().filter(v -> StringUtils.isNotBlank(v.getEditStatus())).collect(Collectors.toList()));
        newTable.setIndexList(newTable.getIndexList().stream().filter(v -> StringUtils.isNotBlank(v.getEditStatus())).collect(Collectors.toList()));

        //update name
        List<TableColumn> columnNameList = newTable.getColumnList().stream().filter(v ->
                v.getOldName() != null && !StringUtils.equals(v.getOldName(), v.getName())).collect(Collectors.toList());
        ...
        List<TableIndex> indexList = newTable.getIndexList().stream().filter(v -> StringUtils.isNotBlank(v.getComment())).collect(Collectors.toList());
        for (TableIndex index : indexList) {
            KingBaseIndexTypeEnum indexEnum = KingBaseIndexTypeEnum.getByType(index.getType());
            script.append(indexEnum.buildIndexComment(index)).append("\n");
        }
