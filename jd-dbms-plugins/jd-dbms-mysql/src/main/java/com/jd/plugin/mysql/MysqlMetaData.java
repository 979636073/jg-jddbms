...
import com.jd.plugin.mysql.builder.MysqlSqlBuilder;
...
public class MysqlMetaData extends DefaultMetaService implements MetaData {
    ...
    public String showViewSql(String databaseName, String schemaName, String viewName, String viewSql, List<TableColumn> columnList) {
        String str = "CREATE   VIEW " + format(viewName) + " AS \n\t" + viewSql + ";\n";
    ...
    private static String VIEW_DDL_SQL="SELECT TABLE_NAME AS `view`, VIEW_DEFINITION FROM INFORMATION_SCHEMA.VIEWS WHERE TABLE_SCHEMA = '%s'  AND TABLE_NAME = '%s'";

    @Override
    public Table view(Connection connection, String databaseName, String schemaName, String viewName) {
        String sql = String.format(VIEW_DDL_SQL,databaseName, viewName);
        return SQLExecutor.getInstance().execute(connection, sql, resultSet -> {
...
//                table.setDdl(resultSet.getString("Create View"));
                ...
                for (TableColumn column : columns) {
                    if (StrUtil.isNotEmpty(column.getComment())) {
                        str += "\n\tCOMMENT ON COLUMN " + format(databaseName) + "." + format(viewName) + "." + format(column.getName()) + " IS '" + column.getComment() + "';";
                    }
                }
                table.setDdl(str);
            }
