...
import com.jd.plugin.gbase.builder.GbaseSqlBuilder;
import com.jd.plugin.gbase.type.*;
...
public class GbaseMetaData extends DefaultMetaService implements MetaData {
    private List<String> systemDatabases = Arrays.asList("information_schema", "performance_schema", "gctmpdb", "gclusterdb", "gbase");
    @Override
    ...
    public String showViewSql(String databaseName, String schemaName, String viewName, String viewSql, List<TableColumn> columnList) {
        String str = "CREATE OR REPLACE VIEW " + format(databaseName) + "." + format(viewName) + " AS \n\t" + viewSql + ";\n";
    ...
    private static String TRIGGER_SQL
            ...
            + "TRIGGER_SCHEMA = '%s' AND TRIGGER_NAME = '%s' AND TABLE_NAME = '%s';";

    private static String TRIGGER_SQL_LIST
            = "select TABLE_NAME as TRIGGER_NAME  from information_schema.TABLES where TRIGGER_SCHEMA = '%s';";
    ...
    public Table view(Connection connection, String databaseName, String schemaName, String viewName) {
        ...
        return SQLExecutor.getInstance().execute(connection, sql, resultSet -> {
            ...
            if (resultSet.next()) {
                table.setViewSql(resultSet.getString("VIEW_DEFINITION"));
            ...
            while (resultSet.next()) {
                ...
                if (tableIndex != null) {
                    ...
                    if ("PRIMARY".equalsIgnoreCase(keyName)) {
                        index.setType(GbaseIndexTypeEnum.PRIMARY_KEY.getName());
                    } else if ("SPATIAL".equalsIgnoreCase(index.getType())) {
                        index.setType(GbaseIndexTypeEnum.SPATIAL.getName());
                    } else if ("FULLTEXT".equalsIgnoreCase(index.getType())) {
                        index.setType(GbaseIndexTypeEnum.FULLTEXT.getName());
                    } else {
                        index.setType(GbaseIndexTypeEnum.NORMAL.getName());
                    }
    ...
    public SqlBuilder getSqlBuilder() {
        return new GbaseSqlBuilder();
    }

    @Override
    public TableMeta getTableMeta(String databaseName, String schemaName, String tableName) {
        return TableMeta.builder()
                .columnTypes(GbaseColumnTypeEnum.getTypes())
                .charsets(GbaseCharsetEnum.getCharsets())
                .collations(GbaseCollationEnum.getCollations())
                .indexTypes(GbaseIndexTypeEnum.getIndexTypes())
                .defaultValues(GbaseDefaultValueEnum.getDefaultValues())
                .build();
    }
