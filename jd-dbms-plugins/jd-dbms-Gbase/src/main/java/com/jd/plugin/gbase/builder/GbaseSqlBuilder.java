package com.jd.plugin.gbase.builder;

import cn.hutool.core.util.ArrayUtil;
import com.jd.plugin.gbase.type.GbaseColumnTypeEnum;
import com.jd.plugin.gbase.type.GbaseIndexTypeEnum;
...
public class GbaseSqlBuilder extends DefaultSqlBuilder {
    @Override
    public String buildCreateTableSql(Table table) {
        StringBuilder script = new StringBuilder();
        script.append("CREATE TABLE ");
        if (StringUtils.isNotBlank(table.getSchemaName())) {
            script.append("`").append(table.getSchemaName()).append("`").append(".");
        }
        ...
        for (TableColumn column : table.getColumnList()) {
            ...
            GbaseColumnTypeEnum typeEnum = GbaseColumnTypeEnum.getByType(column.getColumnType());
        ...
        for (TableIndex tableIndex : table.getIndexList()) {
            ...
            GbaseIndexTypeEnum mysqlIndexTypeEnum = GbaseIndexTypeEnum.getByType(tableIndex.getType());
    ...
    public String buildModifyTaleSql(Table oldTable, Table newTable) {
        ...
        for (TableColumn tableColumn : newTable.getColumnList()) {
            if (StringUtils.isNotBlank(tableColumn.getEditStatus()) && StringUtils.isNotBlank(tableColumn.getColumnType()) && StringUtils.isNotBlank(tableColumn.getName())) {
                GbaseColumnTypeEnum typeEnum = GbaseColumnTypeEnum.getByType(tableColumn.getColumnType());
        ...
        for (TableIndex tableIndex : newTable.getIndexList()) {
            if (StringUtils.isNotBlank(tableIndex.getEditStatus()) && StringUtils.isNotBlank(tableIndex.getType())) {
                GbaseIndexTypeEnum mysqlIndexTypeEnum = GbaseIndexTypeEnum.getByType(tableIndex.getType());
                script.append(mysqlIndexTypeEnum.buildModifyIndex(tableIndex)).append(",\n");
            }
        }
        if (script.length() > 2) {
            script = new StringBuilder(script.substring(0, script.length() - 2));
