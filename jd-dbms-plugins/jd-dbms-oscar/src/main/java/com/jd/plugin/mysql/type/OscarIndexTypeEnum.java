...
//    UNIQUE("Unique", "UNIQUE INDEX"),
    ...
    OscarIndexTypeEnum(String name, String keyword) {
        this.name = name;
    ...
    private String buildDropIndex(TableIndex tableIndex) {
        if (OscarIndexTypeEnum.PRIMARY_KEY.getName().equals(tableIndex.getType())) {
            return StringUtils.join("DROP PRIMARY KEY");
        }
        return StringUtils.join("DROP INDEX ", tableIndex.getOldName(),"");
    }
    public static List<IndexType> getIndexTypes() {
        return Arrays.asList(OscarIndexTypeEnum.values()).stream().map(OscarIndexTypeEnum::getIndexType).collect(java.util.stream.Collectors.toList());
    }
}
