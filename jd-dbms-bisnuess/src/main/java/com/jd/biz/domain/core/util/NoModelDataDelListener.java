...
@Slf4j
public class NoModelDataDelListener extends AnalysisEventListener<Map<Integer, String>> {
    ...
    public NoModelDataDelListener() {
    }

    public NoModelDataDelListener(Db db, String filterName, String filter, String sheetName, List<String> isAutoColumn, Session session, TableImportVo vo) {
        this.db = db;
    ...
    public void invoke(Map<Integer, String> data, AnalysisContext analysisContext) {
        ...
        data.forEach((k, v) -> {
            if (!isAutoColumn.contains(header.get(k))) {
                e.set(header.get(k), v);
            }
        });
        cacheDataList.add(e);
    ...
    private void saveData() {
        try {
            ...
            for (String s1 : s) {
                db.del(sheetName, filterName, s1);
            }
