...
import com.jd.common.filter.PropertyPreExcludeFilter;
...
public class LogAspect
{
    ...
    private static final String[] methods = {"importDmpList", "importDmp", "exportDmp"};
    ...
    protected void handleLog(final JoinPoint joinPoint, Log controllerLog, final Exception e, Object jsonResult)
    {
        try
        {
            ...
            String keyLog = "DMP_LOG:" + userName;
            if (list.contains(methodName)) {
                if (!redisCache.hasKey(keyLog)) {
                    redisCache.setCacheObject(keyLog, Boolean.TRUE);
                    redisCache.expire(keyLog, 20);
                } else {
                    redisCache.expire(keyLog, 20);
                    return;
                }
    ...
    private static String execute_data = "execute_update";
    /**
     * 获取请求的参数，放到log中
     * 
     * @param operLog 操作日志
    ...
    private void setRequestValue(JoinPoint joinPoint, SysOperLog operLog, String[] excludeParamNames) throws Exception
    {
                ...
                && (HttpMethod.PUT.name().equals(requestMethod) || HttpMethod.POST.name().equals(requestMethod)))
        {
            ...
            if (operLog.getOperUrl().contains(execute_data)) {
                if (params.contains(SQLConstants.DELETE)) {
                    params = SQLConstants.getNme(SQLConstants.DELETE) + "\n" + params;
                } else if (params.contains(SQLConstants.INSERT)) {
                    params = SQLConstants.getNme(SQLConstants.INSERT) + "\n" + params;
                } else if (params.contains(SQLConstants.UPDATE)) {
                    params = SQLConstants.getNme(SQLConstants.UPDATE) + "\n" + params;
                }
            }
