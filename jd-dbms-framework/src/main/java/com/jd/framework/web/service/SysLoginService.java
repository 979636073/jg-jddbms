...
import com.easy.cas.client.CasClient;
import com.easy.cas.client.enums.ResultCode;
import com.easy.cas.client.model.SsoDepart;
...
import com.jd.common.exception.user.*;
...
import com.jd.framework.config.AuthorizeUtil;
import com.jd.framework.manager.AsyncManager;
import com.jd.framework.manager.factory.AsyncFactory;
import com.jd.framework.security.context.AuthenticationContextHolder;
...
public class SysLoginService
{
     ...
     * @return 结果
     */
    public String loginOauth(LoginBody loginBody) {
        SsoUser ssoUser = new SsoUser();
        if (isGateway) {
            com.jd.framework.config.SsoUser ssoUserInfo = AuthorizeUtil.getloginuser(loginBody.getTicketToken());
            if (Objects.isNull(ssoUserInfo) || StrUtil.isBlank(ssoUserInfo.getUsername())) {
                throw new BusinessException("未知用户");
            }
            BeanUtils.copyProperties(ssoUserInfo, ssoUser);
        } else {
            Tips tips = CasClient.getLoginUser(loginBody.getTicketToken());
            if (Objects.isNull(tips)) {
                throw new BusinessException("未知用户");
            }
            if(!tips.getCode().equals(ResultCode.OK.getCode())){
                throw new BusinessException(tips.getMessage());
            }
            ssoUser = (SsoUser)tips.getData();
        }
        AsyncManager.me().execute(AsyncFactory.recordLogininfor(ssoUser.getUsername(), Constants.LOGIN_SUCCESS, MessageUtils.message("user.login.success")));
        SysUser sysUser = userService.selectUserByUserName(ssoUser.getUsername());
        if (Objects.isNull(sysUser)) {
            throw new BusinessException("子系统未知用户");
        }
