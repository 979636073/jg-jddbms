   ...
   async navClick(item, params) {
      ...
      if (this.isDialog == "exportArchive") {
        ...
        this.timer = setInterval(() => { 
          sqlServer.exportDmp(params).then((res) => {
            if (res.success && res.data.status == "FINISH") {
              clearInterval(this.timer);
              this.loading = false;
              downloadFile(
    ...
    submitImport() {
      ...
      if (this.isDialog == "importDMP") {
        ...
        if (this.radioImp == "table" && this.radioType == "all") {
          if (!this.formData.user || !this.formData.toUser) {
            return this.$message.error("请输入源用户或目的用户");
          } else {
            const params = {
              fromName: this.formData.user,
              toName: this.formData.toUser,
              fileName: this.fileName,
              filePath: this.url,
              dataSourceId: this.dataInfo.dataSource.id,
            };
            if (this.timeCount) clearInterval(this.timeCount);
            this.loadingTime = true;
            this.timeCount = setInterval(() => {
              sqlServer.importDmp(params).then((res) => {
                if (res.success && res.data.status == "FINISH") {
        ...
        } else if (this.radioImp == "table" && this.radioType == "copyTable") {
          ...
          const params = {
            fromName: this.formData.user,
            toName: this.formData.toUser,
            tableList: this.importCopyTable,
            fileName: this.fileName,
            filePath: this.url,
            dataSourceId: this.dataInfo.dataSource.id,
            schemaName: this.formData.user,
          };
          sqlServer.importDmp(params).then((res) => {
            if (this.timeCount) clearInterval(this.timeCount);
            this.loadingTime = true;
          ...
          if (!this.tableData.length) {
            return this.$message.error("请添加源用户和目的用户!");
          }
          const params = {
            fileName: this.fileName,
            filePath: this.url,
            ...
            toUser: this.fromToUser,
          };
          if (this.timeCount) clearInterval(this.timeCount);
          this.loadingTime = true;
          this.timeCount = setInterval(() => {
            sqlServer.importDmpUser(params).then((res) => {
              if (res.success && res.data.status == "FINISH") {
                clearInterval(this.timeCount);
                this.loadingTime = false;
                if (res.data.msg != "" && res.data.msg != null) {
        ...
        } else if (this.radioImp == "database") {
          const params = {
            fileName: this.fileName,
            filePath: this.url,
            dataSourceId: this.dataInfo.dataSource.id,
    ...
    handleBeforeUpload(file) {
      this.loading2 = true;
      let maxSize = 4 * 1024 * 1024 * 1024 // 4G
      ...
      >
        ...
        <div style="margin-top: 10px; display: flex;flex-direction: column;">
          ...
          >
            ...
            <div style="font-size: 12px;color: #999;">仅支持上传DMP格式文件，文件大小为1G</div>
